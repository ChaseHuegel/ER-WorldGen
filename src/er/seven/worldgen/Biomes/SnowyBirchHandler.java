...
public class SnowyBirchHandler extends ChunkHandler
{
	private FastNoise growthNoise;
	public SnowyBirchHandler()
	{
		growthNoise = new FastNoise();
		growthNoise.SetNoiseType(NoiseType.SimplexFractal);
		growthNoise.SetFrequency(0.04f);
	}
...
//					data.setLayers(  1 + random.nextInt(3) );
		...
		//	Trees
		if (y < 90 && random.nextFloat() <= TreeChance && blockAbove != Material.WATER && (BlockUtil.isDirt(block) || block == Material.SNOW_BLOCK)  )
		{
			GenUtil.GenerateTree(random, area, x, y + 1, z, "ColdBirch", world);
		}
		//	Bushes
