...
public class BlackForestHandler extends ChunkHandler
{
	private FastNoise mudNoise;
	public BlackForestHandler()
	{
		mudNoise = new FastNoise();
		mudNoise.SetNoiseType(NoiseType.SimplexFractal);
		mudNoise.SetFrequency(0.05f);
	}
	
	//	DECORATION
	private static float FoliageChance = 0.5f;
	private static float TreeChance = 0.03f;
	private static float HiveChance = 0.05f;
	private static float RockChance = 0.0075f;
	...
	@Override
	public List<Biome> getValidBiomes() { return Arrays.asList( Biome.TAIGA_MOUNTAINS ); }
		...
		{			
			...
			if (y > 16 || random.nextInt(y) > 8)
			{
				...
				case GRASS_BLOCK:
					...
					if (mudNoise.GetNoise(x, z) > 0.2)
					{
						replacementMaterial = Material.SOUL_SOIL;
					}
					break;
	...
	public void PopulateAt(Random random, int x, int z, DecorationArea area, World world)
	{
		...
		{
			GenUtil.GenerateTree(random, area, x, y + 1, z, "BlackForest", world);
		}
		//	Bushes
		...
		{
			BlockUtil.buildBlob(random, area, x, y, z, 2, 2, 2, Material.MOSSY_COBBLESTONE, Material.COBBLESTONE);
		}
		//	Fallen Logs
